...
async function limpar(orgId, perfisDemo) {
  ...
  // Carteira única (event_id null) das contas de demonstração. Escopo por
  // perfil da demo — nunca "delete from wallets" solto.
  if (perfis.length) {
  ...
  const manter = [SLUG_PASSADO, SLUG_HOJE, SLUG_FUTURO];
...
async function main() {
  ...
  const perfisDemo = [...clientes, ...Object.values(equipe), ...promoters];
  passo(`${perfisDemo.length} contas fictícias prontas (senha única: ${SENHA})`);

  console.log('\n▸ organização…');
  const orgId = await upsertOrg(dono.id);
  const eventosAntes = await limpar(orgId, perfisDemo);
  ...
  passo('9 lotes com preço, meia-entrada e estoque (dois esgotados, dois quase)');
  ...
  passo('5 cupons (ativos, um esgotando, um expirado)');
  ...
  const entradosH = await fazerCheckin(evHoje, vendaH.ingressos, 0.64, equipe['sandro.barreto'].id,
    AGORA - 2 * H, AGORA - 4 * 60e3, { reentradas: 9, saidas: 6, naoVao: clientes.slice(14).map((c) => c.id) });
  ...
  passo(`${carteirasRows.length} carteiras com recargas e consumo (saldo = soma do extrato)`);
  ...
  filaNomes.map((nome, i) => {
    const status = i === 0 ? 'converted' : i === 1 ? 'invited' : i === 8 ? 'expired' : 'waiting';
    return {
  ...
  passo('fila de espera do camarote: 6 aguardando, 1 convidado com prazo correndo, 1 convertido, 1 expirado');

  // ── Notas fiscais ──
  const paraNota = vendaP.pedidos.filter((p) => !p._estornado).slice(0, 14);
  ...
  paraNota.map((p, i) => {
    const status = i < 10 ? 'issued' : i < 12 ? 'pending' : i === 12 ? 'failed' : 'cancelled';
    ...
    return {
      ...
      error: status === 'failed' ? 'Prefeitura recusou: CPF do tomador inválido (código 217)' : null,
      attempts: status === 'failed' ? 3 : status === 'pending' ? 1 : 1,
  ...
  passo('14 notas fiscais: 10 emitidas, 2 na fila, 1 recusada pela prefeitura, 1 cancelada');
  ...
  const [campanhaEnviada] = await inserir('marketing_campaigns',
    ['event_id', 'organization_id', 'subject', 'body', 'segment', 'status', 'mode',
      ...
      { event_id: evHoje, organization_id: orgId,
        ...
        audience_count: 68, sent_count: 0, failed_count: 0, mock_count: 68,
        created_by: dono.id, created_at: iso(AGORA - 9 * H), sent_at: iso(AGORA - 8.5 * H) },
    ], 'id');
  await inserir('marketing_campaigns',
    ['event_id', 'organization_id', 'subject', 'body', 'segment', 'status', 'mode',
      'audience_count', 'sent_count', 'failed_count', 'mock_count', 'created_by', 'created_at', 'sent_at'], [
  ...
  await inserir('marketing_recipients', ['campaign_id', 'email', 'name', 'status', 'created_at', 'sent_at'],
    clientes.concat(clientes).slice(0, 34).map((c, i) => ({
      campaign_id: campanhaEnviada.id, email: i < 18 ? c.email : `convidado${i}${DOMINIO}`,
      name: i < 18 ? c.nome : escolher(NOMES_LISTA), status: 'mock',
      created_at: iso(AGORA - 9 * H), sent_at: iso(AGORA - 8.5 * H),
    })), 'id');
  passo('3 campanhas de marketing (1 enviada hoje em modo mock, 1 rascunho, 1 do evento passado)');
  ...
  await inserir('webhook_entregas', ['assinatura_id', 'evento', 'payload', 'status', 'tentativas',
    'http_status', 'resposta', 'erro', 'proxima_tentativa_em', 'entregue_em', 'created_at'],
  [['pedido.pago', 'entregue', 1, 200], ['ingresso.emitido', 'entregue', 1, 200],
    ['checkin.registrado', 'entregue', 2, 200], ['pedido.estornado', 'desistiu', 6, 502]]
    .map(([evento, status, tentativas, http], i) => ({
      assinatura_id: assinatura.id, evento,
      payload: JSON.stringify({ evento, enviado_em: iso(AGORA - (i + 1) * D), demo: true }),
      status, tentativas, http_status: http,
      resposta: http === 200 ? '{"ok":true}' : null,
      erro: http === 200 ? null : 'HTTP 502 do destino após 6 tentativas',
      proxima_tentativa_em: iso(AGORA - (i + 1) * D),
      entregue_em: status === 'entregue' ? iso(AGORA - (i + 1) * D) : null,
      created_at: iso(AGORA - (i + 1) * D),
    })), 'id');
  passo('integrações: 1 chave de API ativa, 1 revogada e 1 webhook com histórico de entregas');
  ...
  console.log('  · Duas seções ficam vazias porque hoje não há dado possível:');
