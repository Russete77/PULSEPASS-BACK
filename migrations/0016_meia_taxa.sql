-- ═══════════════════════════════════════════════════════════════
-- PulsePass · Migration 0016 — FASE 2 (Sympla): meia-entrada + taxa de serviço
--   • ticket_tiers.half_price_cents  — preço da meia (null = meia indisponível)
--   • events.service_fee_bps         — taxa de serviço em basis points (1000 = 10%)
--   • orders.service_fee_cents       — taxa aplicada, para conciliação
--   • place_order passa a: aceitar item.half, precificar meia, somar a taxa
-- Aplicar após 0001..0015.
-- ═══════════════════════════════════════════════════════════════

alter table public.ticket_tiers add column if not exists half_price_cents integer
  check (half_price_cents is null or half_price_cents >= 0);

alter table public.events add column if not exists service_fee_bps integer not null default 0;
alter table public.events drop constraint if exists chk_service_fee_bps;
alter table public.events add constraint chk_service_fee_bps check (service_fee_bps between 0 and 10000);

alter table public.orders add column if not exists service_fee_cents integer not null default 0;

-- ─────────────────────────────────────────────────────────────────
-- place_order (mesma assinatura/retorno da 0015; só o corpo muda):
--   - item.half=true usa half_price_cents (erro se não configurado);
--   - estoque é compartilhado (meia consome do mesmo quantity_total);
--   - total = subtotal dos ingressos + taxa de serviço (bps do evento).
-- Obs.: o cupom (redeem no orders.service) incide sobre o total (inclui taxa).
-- ─────────────────────────────────────────────────────────────────
